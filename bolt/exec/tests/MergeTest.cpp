 ...
 protected:
  ...
  void testLocalMergeSpill(
    ...
    if (inputBatchSize == 0 || numMaxMergeSources >= numInputSources ||
      ...
      ASSERT_EQ(
          planStats.customStats.count(exec::Merge::kSpilledSourceReadWallNanos),
          0);
      ASSERT_GE(
          planStats.customStats.count(
              exec::Merge::kStreamingSourceReadWallNanos),
          0);
    } else {
...
DEBUG_ONLY_TEST_F(MergeTest, localMergeAbort) {
  ...
  std::atomic_bool callWaitFlag{true};
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::SpillMerger::getOutput",
      std::function<void(std::vector<ContinueFuture>*)>(
...
DEBUG_ONLY_TEST_F(MergeTest, localMergeStart) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::LocalMergeSource::start",
