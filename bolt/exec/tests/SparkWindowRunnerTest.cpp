...
DEFINE_string(
    bolt_testing_window_fuzzer_repro_path,
    "",
...
static std::string checkAndReturnFilePath(
  ...
  auto path = fmt::format(
      "{}/{}", FLAGS_bolt_testing_window_fuzzer_repro_path, fileName);
  if (fs::exists(path)) {
...
static void checkDirForExpectedFiles() {
  LOG(INFO) << "Searching input directory for expected files at "
            << FLAGS_bolt_testing_window_fuzzer_repro_path;
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_bolt_testing_window_fuzzer_repro_path.empty()) {
