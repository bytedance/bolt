...
struct Split {
  ...
  static constexpr int32_t kReuseStringsFromArg = 0;
 ...
 private:
  ...
  void split(
      ...
      const arg_type<Varchar>& delimiter,
      int32_t limit) const {
    BOLT_DCHECK(!delimiter.empty(), "Non-empty delimiter is expected");
    ...
    auto* re = getCachedRegex(delimiter.str());
    const size_t end = input.size();
    const char* start = input.data();
  ...
  re2::RE2* getCachedRegex(const std::string& pattern) const {
    ...
    BOLT_USER_CHECK_LT(
        patternCache_.size(),
        kMaxCachedRegexes,
        "split hit the maximum number of unique regexes: {}",
        kMaxCachedRegexes);

    auto regex = std::make_unique<re2::RE2>(pattern, RE2::Quiet);
    if (UNLIKELY(!regex->ok())) {
      BOLT_USER_FAIL("invalid regular expression:{}", regex->error());
    }
    auto* raw = regex.get();
    patternCache_.emplace(pattern, std::move(regex));
    return raw;
  }

  static constexpr int32_t kMaxCachedRegexes = 40;
