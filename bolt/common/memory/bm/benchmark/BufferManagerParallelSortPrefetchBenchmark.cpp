...
DEFINE_string(
    ...
    "/tmp/bolt-bm-parallel-sort-prefetch-benchmark",
    "Directory used by parallel BufferManager sort prefetch benchmark spill files.");
DEFINE_uint64(
...
DEFINE_uint64(
    ...
    8,
    "Number of blocks requested by each generation-stage BatchAllocate call.");
DEFINE_uint64(
    bm_parallel_sort_prefetch_distance,
    4,
    "Number of next blocks each run cursor submits through Prefetch.");
...
uint64_t divideRoundUp(uint64_t value, uint64_t divisor) {
  BOLT_CHECK_GT(divisor, 0);
  return (value + divisor - 1) / divisor;
}

uint64_t checkedMultiply(uint64_t lhs, uint64_t rhs) {
  if (lhs != 0) {
    BOLT_CHECK_LE(rhs, std::numeric_limits<uint64_t>::max() / lhs);
  }
  return lhs * rhs;
}
...
AllocateSize parseAllocateSize(const std::string& value) {
  ...
  throw std::invalid_argument(
      "bm_parallel_sort_prefetch_allocate_size must be small, medium, or large");
}
...
struct PrefetchingRunCursor {
  SortedRun* run{nullptr};
  BufferManager* manager{nullptr};
  size_t workerIndex{0};
  size_t runIndex{0};
  size_t nextBlockIndex{0};
  size_t submittedPrefetchIndex{0};
  size_t prefetchDistance{0};
  uint64_t emitted{0};
  PinnedRunBlock pinned;

  PrefetchingRunCursor(
      SortedRun& sortedRun,
  ...
  bool hasNextBlock() const {
    return nextBlockIndex < run->blocks.size();
  }

  std::shared_ptr<BlockHandle> takeNextBlock() {
    if (!hasNextBlock()) {
      return nullptr;
    }
    auto& block = run->blocks[nextBlockIndex++];
    BOLT_CHECK_NOT_NULL(block.block);
    submittedPrefetchIndex = std::max(submittedPrefetchIndex, nextBlockIndex);
    return std::move(block.block);
  }

  void installPinnedBlock(BufferHandle handle, size_t blockIndex) {
    pinned = {};
    pinned.handle = std::move(handle);
    pinned.values = reinterpret_cast<uint64_t*>(pinned.handle.Ptr());
    pinned.count = run->blocks[blockIndex].values;
    pinned.index = 0;
    submitPrefetchWindow();
  }

  void submitPrefetchWindow() {
    ...
    while (submittedPrefetchIndex < target) {
      auto& block = run->blocks[submittedPrefetchIndex++];
...
struct WorkerResult {
  ...
  size_t runCount{0};
  size_t blockCount{0};
  uint64_t batchAllocateCalls{0};
  uint64_t batchAllocateRequestedBlocks{0};
  uint64_t batchAllocateReturnedBlocks{0};
...
void appendAllocatedBatchToActiveRun(
    ...
    ActiveRun& active,
    WorkerResult& result) {
  ++result.batchAllocateCalls;
  result.batchAllocateRequestedBlocks += batchBlocks;
  auto handles =
      manager.BatchAllocate(batchBlocks, blockBytes, MemoryTag::kUnknown);
  result.batchAllocateReturnedBlocks += handles.size();
...
bool verifySortedRuns(
  ...
  std::vector<PrefetchingRunCursor> cursors;
  cursors.reserve(runs.size());
  std::vector<InitialPin> initialPins;
  initialPins.reserve(runs.size());
  ...
  std::vector<std::shared_ptr<BlockHandle>> firstBlocks;
  firstBlocks.reserve(initialPins.size());
  ...
  BOLT_CHECK_EQ(firstHandles.size(), initialPins.size());
  for (size_t i = 0; i < initialPins.size(); ++i) {
    auto& pin = initialPins[i];
    cursors[pin.cursor].installPinnedBlock(
        std::move(firstHandles[i]), pin.blockIndex);
    heap.push(HeapEntry{cursors[pin.cursor].value(), pin.cursor});
  }
  firstBlocks.clear();
  initialPins.clear();
...
WorkerResult runWorker(
  ...
  config.poolName = "bm-parallel-sort-prefetch-" + std::to_string(workerIndex);
...

  {
    ...
    while (!generator.empty()) {
      const auto remainingValues = result.totalValues - generator.generated();
      const auto remainingBlocks =
          divideRoundUp(remainingValues, valuesPerBlock);
      const auto batchBlocks = static_cast<size_t>(
          std::min<uint64_t>(allocateBatchBlocks, remainingBlocks));
      const auto batchBytes = checkedMultiply(batchBlocks, blockBytes);

      if (!manager->MaybeReserve(batchBytes) && !active.empty()) {
      ...
      appendAllocatedBatchToActiveRun(
          *manager,
          generator,
          valuesPerBlock,
          blockBytes,
...
int runBenchmark() {
  const auto threadCount =
      static_cast<size_t>(FLAGS_bm_parallel_sort_prefetch_threads);
  ...
  for (size_t i = 0; i < threadCount; ++i) {
    contexts.push_back(std::make_unique<SparkListenableArbitratorContext>(
        SparkListenableArbitratorContextOptions{
            .name = "bm-parallel-sort-prefetch-benchmark-" + std::to_string(i),
        ...
        try {
          results[i] = runWorker(
              ...
              *roots[i],
              allocateSize,
              dataBytes,
              FLAGS_bm_parallel_sort_prefetch_seed,
              spillRoot,
              allocateBatchBlocks,
              prefetchDistance);
        } catch (...) {
  ...
  uint64_t aggregateBatchAllocateCalls = 0;
  uint64_t aggregateBatchAllocateRequestedBlocks = 0;
  uint64_t aggregateBatchAllocateReturnedBlocks = 0;
