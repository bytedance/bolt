...
TEST_F(ParquetReaderTest, decimalReaderCastValueFilters) {
  ...
  auto rowReader =
      createFilteredReader(DECIMAL(15, 2), exec::equal(int64_t{100}));
  auto result = BaseVector::create(ROW({"c0"}, {DECIMAL(15, 2)}), 0, pool());
  EXPECT_EQ(rowReader->next(10, result), 3);
  ASSERT_EQ(result->size(), 1);
  EXPECT_EQ(
      loadedFlatChildAt<int64_t>(result->as<RowVector>(), 0)->valueAt(0), 100);

  // A value-dependent requested-scale filter must not reach file-scale data.
  BOLT_ASSERT_THROW(
      createFilteredReader(DECIMAL(15, 4), exec::equal(int64_t{10'000})),
      "Cannot apply BIGINT filter to physical BIGINT Parquet column c0");

  // Value-independent filters remain safe across decimal reader casts.
  rowReader = createFilteredReader(DECIMAL(15, 4), exec::isNull());
  result = BaseVector::create(ROW({"c0"}, {DECIMAL(15, 4)}), 0, pool());
  EXPECT_EQ(rowReader->next(10, result), 3);
  ASSERT_EQ(result->size(), 1);
