...
namespace bytedance::bolt::connector::tpcds::test {
namespace {
...
TEST_F(TpcdsConnectorTest, simple) {
  auto plan =
      exec::test::PlanBuilder()
          .tpcdsTableScan(
              bolt::tpcds::Table::TBL_WAREHOUSE,
  ...
  bolt::test::assertEqualVectors(expected, output);
}
// Extract single column from table 'store'.
TEST_F(TpcdsConnectorTest, singleColumn) {
  auto plan =
      exec::test::PlanBuilder()
          .tpcdsTableScan(bolt::tpcds::Table::TBL_STORE, {"s_store_name"}, 1)
          .planNode();
  auto output = getResults(plan, {makeTpcdsSplit()});
...
TEST_F(TpcdsConnectorTest, singleColumnWithAlias) {
  ...
  auto plan = exec::test::PlanBuilder()
                  ...
                  .tableHandle(std::make_shared<TpcdsTableHandle>(
                      kTpcdsConnectorId, bolt::tpcds::Table::TBL_ITEM))
                  .assignments({
                      {aliasedName,
...
TEST_F(TpcdsConnectorTest, unknownColumn) {
  EXPECT_THROW(
      {
        exec::test::PlanBuilder()
            .tpcdsTableScan(bolt::tpcds::Table::TBL_ITEM, {"invalid_column"})
            .planNode();
      },
      BoltUserError);
}
  ...
  auto plan =
      exec::test::PlanBuilder(planNodeIdGenerator)
          ...
          .hashJoin(
              ...
              {"s_state"},
              exec::test::PlanBuilder(planNodeIdGenerator)
                  .tpcdsTableScan(bolt::tpcds::Table::TBL_STORE, {"s_state"}, 1)
...
TEST_F(TpcdsConnectorTest, inventoryDateCount) {
  auto plan =
      exec::test::PlanBuilder()
          .tpcdsTableScan(
              bolt::tpcds::Table::TBL_WEB_SITE, {"web_rec_start_date"}, 1)
