...
TEST_F(PaimonConnectorTest, TestTableScanDataEvolution) {
  ...
  auto rootPool = memory::memoryManager()->addRootPool("PaimonConnectorTest");
  auto leafPool = rootPool->addLeafChild("leaf");
  auto schema = ROW({"id", "value", "length"}, {BIGINT(), VARCHAR(), INTEGER()});
  ...
  // Build table path using the temporary directory
  std::string tablePath = "file:" + tempDir_->path + "/test_db.db/data_evolution";
  ...
  // Define schema and handles
  auto rowType = ROW({"id", "value", "length"}, {BIGINT(), VARCHAR(), INTEGER()});
  std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>
      columnHandles;
  columnHandles["id"] = std::make_shared<PaimonColumnHandle>("id", BIGINT());
  columnHandles["value"] = std::make_shared<PaimonColumnHandle>("value", VARCHAR());
  columnHandles["length"] = std::make_shared<PaimonColumnHandle>("length", INTEGER());

  auto tableHandle = std::make_shared<PaimonTableHandle>(
      "paimon_test", "test_table", tablePath, std::unordered_map<std::string, std::string>({{::paimon::Options::ROW_TRACKING_ENABLED, "true"}, {::paimon::Options::DATA_EVOLUTION_ENABLED, "true"}}));
...
TEST_F(PaimonConnectorTest, TestTableScanPartialUpdate) {
  ...
  auto rowType = ROW({"id", "name", "age", "salary"}, {BIGINT(), VARCHAR(), INTEGER(), DOUBLE()});
  ...
  // Build table path using the temporary directory
  std::string tablePath = "file:" + tempDir_->path + "/test_db.db/partial_update";
  ...
  std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>
      columnHandles;
  columnHandles["id"] = std::make_shared<PaimonColumnHandle>("id", BIGINT());
  columnHandles["name"] = std::make_shared<PaimonColumnHandle>("name", VARCHAR());
  columnHandles["age"] = std::make_shared<PaimonColumnHandle>("age", INTEGER());
  columnHandles["salary"] = std::make_shared<PaimonColumnHandle>("salary", DOUBLE());
...
TEST_F(PaimonConnectorTest, TestTableScanAggregate) {
  ...
  std::unique_ptr<::paimon::ScanContext> scanContext =
                           contextBuilder
                               ...
                               .Finish()
                               .value();
    auto tableScanResult =
                           ::paimon::TableScan::Create(std::move(scanContext));
    BOLT_CHECK(tableScanResult.ok(), "Failed to create table scan: {}", tableScanResult.status().ToString());
    const auto& tableScan = tableScanResult.value();
    const auto& scanPlanResult = tableScan->CreatePlan();
    BOLT_CHECK(scanPlanResult.ok(), "Failed to create plan: {}", scanPlanResult.status().ToString());
    std::shared_ptr<::paimon::Plan> paimonPlan =
                           scanPlanResult.value();

  // Define schema and handles
  std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>
      columnHandles;
  columnHandles["id"] = std::make_shared<PaimonColumnHandle>("id", BIGINT());
  columnHandles["sales"] = std::make_shared<PaimonColumnHandle>("sales", BIGINT());
  columnHandles["price"] = std::make_shared<PaimonColumnHandle>("price", DOUBLE());
...
TEST_F(PaimonConnectorTest, TestTableScanDeduplicate) {
  ...
  auto rowType = ROW({"id", "value", "timestamp"}, {BIGINT(), VARCHAR(), BIGINT()});

  const int64_t kRows = 3;
  std::vector<int64_t> ids = {1, 2, 3, 4};
  std::vector<std::string> values = {"v1", "v2_updated", "v3_updated", "v4_updated"};
  ...
  std::unique_ptr<::paimon::ScanContext> scanContext =
    ...
    BOLT_CHECK(paimonPlanResult.ok(), "Failed to create plan: {}", paimonPlanResult.status().ToString());
  ...
  columnHandles["id"] = std::make_shared<PaimonColumnHandle>("id", BIGINT());
  columnHandles["value"] = std::make_shared<PaimonColumnHandle>("value", VARCHAR());
  columnHandles["timestamp"] = std::make_shared<PaimonColumnHandle>("timestamp", BIGINT());
