...
DEFINE_string(
    bolt_testing_complex_constant_path,
    "",
...
DEFINE_string(
    bolt_testing_sql,
    "",
    "Comma separated SQL expressions to evaluate. This flag and --bolt_testing_sql_path "
    "flag are mutually exclusive. If both are specified, --bolt_testing_sql is used and "
    "--bolt_testing_sql_path is ignored.");

DEFINE_string(
    bolt_testing_result_path,
    "",
...
DEFINE_string(
    ...
    "results.\n"
    "query: evaluate SQL query specified in --bolt_testing_sql or --bolt_testing_sql_path and print out "
    "results. If --bolt_testing_input_path is specified, the query may reference it as "
    "table 't'.");

DEFINE_string(
    bolt_testing_lazy_column_list_path,
    "",
...
DEFINE_bool(
    bolt_testing_find_minimal_subexpression,
    false,
...
static std::string checkAndReturnFilePath(
  ...
  auto path =
      fmt::format("{}/{}", FLAGS_bolt_testing_fuzzer_repro_path, fileName);
  if (fs::exists(path)) {
...
static void checkDirForExpectedFiles() {
  LOG(INFO) << "Searching input directory for expected files at "
            << FLAGS_bolt_testing_fuzzer_repro_path;

  FLAGS_bolt_testing_input_path = FLAGS_bolt_testing_input_path.empty()
      ? checkAndReturnFilePath(
            test::ExpressionVerifier::kInputVectorFileName, "input_path")
      : FLAGS_bolt_testing_input_path;
  FLAGS_bolt_testing_result_path = FLAGS_bolt_testing_result_path.empty()
      ? checkAndReturnFilePath(
            test::ExpressionVerifier::kResultVectorFileName, "result_path")
      : FLAGS_bolt_testing_result_path;
  FLAGS_bolt_testing_sql_path = FLAGS_bolt_testing_sql_path.empty()
      ? checkAndReturnFilePath(
            test::ExpressionVerifier::kExpressionSqlFileName, "sql_path")
      : FLAGS_bolt_testing_sql_path;
  FLAGS_bolt_testing_lazy_column_list_path =
      FLAGS_bolt_testing_lazy_column_list_path.empty()
      ? checkAndReturnFilePath(
            test::ExpressionVerifier::kIndicesOfLazyColumnsFileName,
            "lazy_column_list_path")
      : FLAGS_bolt_testing_lazy_column_list_path;
  FLAGS_bolt_testing_complex_constant_path =
      FLAGS_bolt_testing_complex_constant_path.empty()
      ? checkAndReturnFilePath(
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_bolt_testing_fuzzer_repro_path.empty()) {
    checkDirForExpectedFiles();
  }

  if (FLAGS_bolt_testing_sql.empty() && FLAGS_bolt_testing_sql_path.empty()) {
    std::cout
        << "One of --bolt_testing_sql or --bolt_testing_sql_path flags must be set."
        << std::endl;
    exit(1);
  }

  auto sql = FLAGS_bolt_testing_sql;
  if (sql.empty()) {
    sql = restoreStringFromFile(FLAGS_bolt_testing_sql_path.c_str());
  ...
  test::ExpressionRunner::run(
      ...
      FLAGS_bolt_testing_complex_constant_path,
      FLAGS_bolt_testing_result_path,
      FLAGS_bolt_testing_mode,
      FLAGS_bolt_testing_num_rows,
      FLAGS_bolt_testing_store_result_path,
      FLAGS_bolt_testing_lazy_column_list_path,
      FLAGS_bolt_testing_find_minimal_subexpression);
}
